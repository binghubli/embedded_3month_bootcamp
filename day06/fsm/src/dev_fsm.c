...
static void state_idle(DevFsmCtx* ctx, DevEvent evt)
{
    ctx->tick_cnt = 0;
    if (evt == EV_START)
        printf("[TABLE FSM] IDLE -> WORKING(开始工作)\n");
    if (evt == EV_ERR)
    {
        printf("[TABLE FSM] IDLE -> ERROR(空闲异常)\n");
    }
}
...
static void state_working(DevFsmCtx* ctx, DevEvent evt)
{
    ...
    if (evt == EV_STOP)
    {
        printf("[TABLE FSM] WORKING -> IDLE(停止工作)\n");
    }
    if (evt == EV_ERR)
    {
        printf("[TABLE FSM] WORKING -> ERROR(工作异常)\n");
    }
    if (evt == EV_TIMEOUT)
    {
        printf("[TABLE FSM] WORKING -> ERROR(工作超时)\n");
    }
}
...
static void state_error(DevFsmCtx* ctx, DevEvent evt)
{
    ...
    if (evt == EV_RESET)
    {
        printf("[TABLE FSM] ERROR -> RECOVERY(故障恢复)\n");
    }
}
...
static void state_recovery(DevFsmCtx* ctx, DevEvent evt)
{
    ...
    if (evt == EV_POWER_ON)
    {
        printf("[TABLE FSM] RECOVERY -> IDLE(故障恢复完成)\n");
    }
    if (evt == EV_TIMEOUT)
    {
        printf("[TABLE FSM] RECOVERY -> ERROR(故障恢复超时)\n");
    }
}
